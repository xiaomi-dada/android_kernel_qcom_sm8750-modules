...
/* The board whose element supply is shared and therefore counted. */
#define ST54SPI_COUNTED_PRODUCT "xuanyuan"
#define ST54SPI_POWER_ON_SETTLE_US  10000
#define ST54SPI_POWER_OFF_SETTLE_US 3000
...
struct st54spi_gpio_device {
	...
	int gpiod_reset;
	/* How many users are holding the element powered, where counted */
	int power_count;
};
...
long st54spi_gpio_dev_ioctl(struct file *pfile, unsigned int cmd, unsigned long arg)
{
	...
	case ST54SPI_SET_GPIO:
		...
		/*
		 * One board wires the element's supply so that more than one
		 * user can hold it up, and the pin may only drop when the last
		 * of them lets go.  Everywhere else a request to power down is
		 * ignored and the pin is only ever driven high.
		 */
		if (strcmp(product_name_get(), ST54SPI_COUNTED_PRODUCT)) {
			if (arg == 1)
				gpio_set_value(st54spi_gpio_dev->gpiod_reset, 1);
			break;
		}

		pr_info("Original power count = %d\n",
			st54spi_gpio_dev->power_count);
		if (arg)
			st54spi_gpio_dev->power_count++;
		else
			st54spi_gpio_dev->power_count--;
		pr_info("Updated power count = %d\n",
			st54spi_gpio_dev->power_count);

		if (st54spi_gpio_dev->power_count < 0) {
			st54spi_gpio_dev->power_count = 0;
		} else if (!arg && st54spi_gpio_dev->power_count) {
			pr_info("No execution power count = %d\n",
				st54spi_gpio_dev->power_count);
			break;
		}

		pr_info("GPIO setting for power count = %d\n",
			st54spi_gpio_dev->power_count);
		gpio_set_value(st54spi_gpio_dev->gpiod_reset, arg);
		/* The supply needs longer to come up than to fall away. */
		if (arg)
			usleep_range(ST54SPI_POWER_ON_SETTLE_US,
				     ST54SPI_POWER_ON_SETTLE_US + 1);
		else
			usleep_range(ST54SPI_POWER_OFF_SETTLE_US,
				     ST54SPI_POWER_OFF_SETTLE_US + 1);
		break;
	default:
