...
// QTI and MTK54 use standard GPIO definition
	...
	if (IS_ERR_OR_NULL(st21nfc_dev->gpiod_clkreq)) {
		...
		/*
		 * The clock request line has to stay able to wake the SoC while
		 * the rest of it is asleep, otherwise the controller cannot ask
		 * for its clock back.  The GPIO number is the second cell of
		 * the phandle, which the descriptor does not carry.
		 */
		ret = of_property_read_u32_index(dev->of_node, "clkreq-gpios", 1,
						 &clkreq_gpio);
		if (ret < 0) {
