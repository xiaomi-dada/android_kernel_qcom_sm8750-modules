...
static void dump(void)
{
	...
	int ret = 0;
	pr_debug(
		"----------------- DUMP CAMERALOG START------------------------");
	pr_debug("cameralog version %s", CAMLOG_DEV_VERSION);

	pr_debug(" last writer uid:%u  pid:%u", s_last_caller_uid,
		 s_last_caller_pid);
	pr_debug(" last reader uid:%u  pid:%u", s_last_reader_uid,
		 s_last_reader_pid);
	...
	if (message_cnt > 0) {
		...
		for (i = 0; i < message_cnt; i++) {
			ret = kfifo_out(&message_kfifo, &element,
					sizeof(struct camlog_dev));
			if (ret != sizeof(struct camlog_dev)) {
				break;
			}
			pr_debug("  %u\t%u\t %s\n", element.writer_uid,
				 element.writer_pid, element.m_camlog_message);
		}
		pr_debug("-------------------------------");
	}
	pr_debug(
			    ...
			    unsigned long arg)
{
	...
	case IOCTL_CMD_READ_LAST_MSG: {
		if (copy_to_user((char *)arg, s_last_message,
				 strlen(s_last_message) + 1)) {
			return -EFAULT;
		}
	} break;
	default:
			      ...
			      loff_t *ppos)
{
	...
	if (debugLevel > 0) {
		pr_debug("cameralog_read  kfifo len:%lu",
			 kfifo_len(&message_kfifo) / sizeof(struct camlog_dev));
	}
	...
	if (ret != sizeof(struct camlog_dev)) {
		if (debugLevel > 0) {
			pr_debug(
				"cameralog_read: not read anything from kfifo %d  expect:%lu\n",
	...
	if (debugLevel > 0) {
		pr_debug("cameralog_read message to user:%s   len:%d",
			 s_read_buf.m_camlog_message, message_len);
	}
	...
	if (debugLevel > 0) {
		pr_debug("cameralog_read return len:%zd", bytes_read);
	}
			       ...
			       loff_t *ppos)
{
	...
	if (debugLevel > 0) {
		pr_debug("cameralog_write  valid_len:%d  strlen:%zd.",
			 valid_len, bytes_wirte);
	}
	if (0 == bytes_wirte) {
		//empty string!
		mutex_unlock(&camlog_message_lock);
	...
	if (ret != sizeof(struct camlog_dev)) {
		if (debugLevel > 0) {
			pr_debug(
				"cameralog: Failed to write to kfifo %d -  %lu\n",
				ret, sizeof(struct camlog_dev));
		}
		mutex_unlock(&camlog_message_lock);
		return 0;
	...
	if (debugLevel > 0) {
		pr_debug("cameralog message kfifo   len:%d",
			 kfifo_len(&message_kfifo));
	}
	mutex_unlock(&camlog_message_lock);
...
static int __init cameralog_init(void)
{
	...
	int ret = 0;
	ret = kfifo_alloc(&message_kfifo,
			  KFIFO_COUNT * sizeof(struct camlog_dev), GFP_KERNEL);
	if (ret) {
		printk(KERN_ERR
		       "cameralog message kfifo: Failed to allocate kfifo\n");
		return ret;
	}
	camlog_major = register_chrdev(0, CAMLOG_DEV_NAME, &cameralog_fops);
	camlog_class = class_create(CAMLOG_DEV_NAME);
	...
	device = device_create(camlog_class, NULL, devt, NULL, CAMLOG_DEV_NAME);
	if (IS_ERR(device)) {
